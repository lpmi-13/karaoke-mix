...
The development server listens on `127.0.0.1` only, so it is not exposed to
...
`netlify.toml` applies a restrictive Content Security Policy and browser
security headers to every Netlify response. The policy retains the existing
Google Fonts stylesheet; self-host those font files before removing the Google
style and font origins from the policy.
...
enforce pinned byte sizes, and rename only after verification. Source URLs are
restricted to the configured MetaBrainz HTTPS hosts, archive extraction keeps a
free-space reserve, and ListenBrainz responses have a hard size limit. Each
completed build stage is fingerprinted in DuckDB. ListenBrainz requests use
deterministic batches, a meaningful user agent, exponential retry for rate
limits/transient failures, and per-batch checkpoints.

Python requirements are hash-locked in `requirements-catalog.txt`. To update
them after editing `requirements-catalog.in`, run:

```bash
uv pip compile requirements-catalog.in --generate-hashes --output-file requirements-catalog.txt
```
